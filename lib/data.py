...
def get_dataset(dset_name, batch_size, n_worker, data_root='../../data'):
    ...
    if dset_name == 'cifar10':
        ...
        train_loader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True,
                                                   num_workers=n_worker, pin_memory=True, sampler=None,drop_last=True)
        ...
        transform_train = transforms.Compose(cifar100_tran_train)
        transform_test = transforms.Compose(cifar100_tran_test)
        ...
        train_loader = torch.utils.data.DataLoader(
            ...
            batch_size=batch_size, shuffle=True,
            num_workers=n_worker, pin_memory=True, sampler=None)
        print(len(datasets.ImageFolder(traindir, transforms.Compose(imagenet_tran_train))))
        val_loader = torch.utils.data.DataLoader(
            datasets.ImageFolder(valdir, transforms.Compose(imagenet_tran_test)),
            batch_size=batch_size, shuffle=False,
            num_workers=n_worker, pin_memory=True)
        n_class = 25
    elif dset_name == 'tiny-imagenet':
        traindir = os.path.join(data_root, 'train')
        valdir = os.path.join(data_root, 'val')
        input_size = 224
        tiny_tran_train = [
        ...
        train_loader = torch.utils.data.DataLoader(
            datasets.ImageFolder(traindir, transforms.Compose(tiny_tran_train)),
            batch_size=batch_size, shuffle=True,
            num_workers=n_worker, pin_memory=True, sampler=None)

        val_loader = torch.utils.data.DataLoader(
            datasets.ImageFolder(valdir, transforms.Compose(tiny_tran_test)),
            batch_size=batch_size, shuffle=False,
            num_workers=n_worker, pin_memory=True)
        n_class = 200
